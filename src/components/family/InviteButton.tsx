  ...
  return (
    <button
      ...
      onClick={async () => {
        ...
        if (!uid) { setBusy(false); onToast?.("Syncing…"); return; }
        const { error } = await supabase
          .from("family_invites")
          .insert({ family_id: familyId, code, created_by: uid });
        if (error) {
          console.error("[invite]", error);
          onToast?.("Couldn't create invite");
          setBusy(false);
          return;
        }
        window.open(whatsappInviteUrl({ familyName, code, budget }), "_blank");
        onToast?.(`Invite ${code} ready`);
        setBusy(false);
      }}
      className={
        className ??
        "inline-flex items-center gap-2 px-4 py-2.5 rounded-full font-medium text-sm text-white shadow-md transition active:scale-95 disabled:opacity-60"
      }
